...
// PATCH — Update retailer (name, password, retail_pin, is_active)
...
// DELETE — Remove retailer (only if no customers)
export async function DELETE(req: NextRequest) {
  ...
  if (count && count > 0) {
    return NextResponse.json({ error: `Cannot delete: ${count} customer(s) assigned to this retailer. Reassign or delete them first.` }, { status: 409 });
  }
  ...
  if (retailer?.auth_user_id) {
    await serviceClient.auth.admin.deleteUser(retailer.auth_user_id);
  }
